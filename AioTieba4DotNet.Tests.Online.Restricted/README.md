...
按 API 过滤（只针对矩阵里已经直接覆盖、且已声明为 `restricted-api(...)` 的入口）：

```bash
dotnet test AioTieba4DotNet.Tests.Online.Restricted/AioTieba4DotNet.Tests.Online.Restricted.csproj --configuration Release --nologo --filter "TestCategory=Api:Threads.DelThreadAsync&TestCategory=Tier:Restricted"
...
dotnet test AioTieba4DotNet.Tests.Online.Restricted/AioTieba4DotNet.Tests.Online.Restricted.csproj --configuration Release --nologo --filter "Name=DefaultLocalEnvironmentStopsBeforeRestrictedAdminMutationAttempt"
```
...
- API 级别过滤只适用于 `docs/related/public-api-coverage-matrix.md` 里已经有直接 Restricted 证据、且验证命令明确写成 `restricted-api(...)` 的入口；只有在你明确要点名单个测试方法时才再退回 `Name=`。
- API 分类同样使用模块限定名，避免与 Safe 项目里的同名方法混淆，例如 `Api:Threads.DelPostAsync` 和 `Api:Admins.BlockAsync` 分别对应不同模块的稳定过滤面。
- 不是所有 Restricted 公开 API 都已首类可过滤；像 `AddBawuAsync`、`DelBawuAsync`、`GetBawuPermAsync` 这类仍无直接场景证据的行，目前只保留在矩阵里，不宣称 `Api:*` 过滤入口。

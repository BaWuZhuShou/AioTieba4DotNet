...
- 先判断是否需要登录态；需要写操作时，默认使用 `new TiebaClient("你的BDUSS", "你的STOKEN")`。
...
这个 skill 是独立导出的。无论是放进 skill registry，还是走类似安装的流程，都应该整体分发 `aiotieba4dotnet/` 目录。

如果要做成更接近包安装的分发方式，使用同目录下的 `package.json` 作为最小发布元信息：

- 包名：`@bawuzhushou/aiotieba4dotnet-skill`
- skill 入口：`SKILL.md`
- 机器可读 manifest：`skill.json`
- 打包内容：`SKILL.md` 与 `references/`
- 自定义字段：`opencodeSkill`
- 版本策略：skill 自己独立走 SemVer，不跟 NuGet 包版本强绑定

推荐安装器优先读取 `skill.json` 获取 skill 元信息，再用 `package.json` 获取包级发布信息。

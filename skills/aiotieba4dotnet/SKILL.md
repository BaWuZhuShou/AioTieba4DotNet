...
这个 skill 是独立导出的，面向支持 `vercel-labs/skills` 风格仓库来源添加的工具。

文档里应统一使用仓库来源添加方式：

- 仓库来源：`npx skills add BaWuZhuShou/AioTieba4DotNet`
- GitHub 目录来源：`npx skills add https://github.com/BaWuZhuShou/AioTieba4DotNet/tree/master/skills/aiotieba4dotnet`

相关元信息保持如下：

- skill 入口：`SKILL.md`
- 机器可读 manifest：`skill.json`
- 打包内容：`skill.json`、`SKILL.md` 与 `references/`
- 自定义字段：`opencodeSkill`

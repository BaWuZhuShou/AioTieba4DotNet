...
[TestClass]
public class ThreadWriteProtocolTests
{
    [TestMethod]
    ...
    [TestMethod]
    public async Task DelThreadsAsync_RejectsNullAndEmptyBatchBeforeForumLookup()
    {
        var forum = new CountingForumProtocol();
        var httpCore = new RecordingHttpCore();
        var protocol = CreateProtocol(CreateAuthenticatedSession(httpCore), forum);
    ...
    [TestMethod]
    public async Task DelPostsAsync_RejectsNonPositiveTargetBeforeForumLookup()
    {
        var forum = new CountingForumProtocol();
        var httpCore = new RecordingHttpCore();
        var protocol = CreateProtocol(CreateAuthenticatedSession(httpCore), forum);
    ...
    public async Task GoodAsync_ResolvesCategoryBeforeCommit()
    {
        ...
        using var session = CreateAuthenticatedSession(httpCore);
        var forumCache = new ForumInfoCache();
        forumCache.SetForumName(3581744UL, "lol欧服");
        var protocol = new ThreadProtocol(
            new TiebaOperationDispatcher(session),
            new ForumProtocol(new TiebaOperationDispatcher(session), forumCache));
    ...
    [TestMethod]
    public async Task RecoverAsync_RejectsMissingAndAmbiguousTargetsBeforeForumLookup()
    {
        var forum = new CountingForumProtocol();
        var httpCore = new RecordingHttpCore();
        var protocol = CreateProtocol(CreateAuthenticatedSession(httpCore), forum);
    ...
    private static ThreadProtocol CreateProtocol(TiebaClientSession session, CountingForumProtocol forumProtocol)
    {

...
public class ThreadModerationApiTests
{
    ...
    public async Task DelThread_RequestAsync_PacksBawuDeleteThreadForm()
    {
        ...
        var success = await api.RequestAsync(3581744, 10377929712, isHide: false);
    ...
    public async Task BatchDeleteApis_PackJoinedIdsAndBlockMode()
    {
        ...
        await deleteThreads.RequestAsync(3581744, new long[] { 1, 2, 3 }, block: true);
        ...
        await deletePosts.RequestAsync(3581744, 99, new long[] { 11, 22 }, block: false);
    ...
    public async Task TopAndMove_RequestAsync_PackExpectedPayloads()
    {
        ...
        await top.RequestAsync("lol欧服", 3581744, 10377929712, isVip: true, isSet: false);
    ...
    public async Task Recommend_RequestAsync_ThrowsWhenPushFails()
    {
        var httpCore = new RecordingHttpCore
        {
            AppFormResponse = "{\"error_code\":0,\"error_msg\":\"\",\"data\":{\"is_push_success\":0,\"msg\":\"denied\"}}"
        };
    ...
    public async Task Recommend_RequestAsync_ReturnsTrueWhenPushSucceeds()
    {
        var httpCore = new RecordingHttpCore
        {
            AppFormResponse = "{\"error_code\":0,\"error_msg\":\"\",\"data\":{\"is_push_success\":1,\"msg\":\"ok\"}}"
        };
        var api = new Recommend(httpCore);
    ...
    public async Task RecoverAndSetThreadPrivacy_RequestAsync_PackOwnedTargetFields()
    {
        ...
        await recover.RequestAsync(3581744, 0, 153071185710, isHide: false);
        ...
        await privacy.RequestAsync(3581744, 10377929712, 153071185710, isPrivate: true);
    ...
    private sealed class RecordingHttpCore : ITiebaHttpCore
    {
        ...
        public string GetAppFormValue(string key) =>
            LastAppFormData.Last(entry => entry.Key == key).Value;

        public string? GetOptionalAppFormValue(string key) =>
            LastAppFormData.LastOrDefault(entry => entry.Key == key).Value;

...
`docs/related/parity.md` 已经承担 upstream parity truth，但 v3 仍需要一个**明确的产品契约定义**：

- 当前仓库仍然多目标构建 `net8.0` / `net9.0` / `net10.0`
- 现有文档仍主要描述 v2 故事
- 仓库仍需要围绕 build、coverage、session/transport、public API、docs 和 migration 做持续实现与治理
- 如果支持矩阵、coverage scope 和 public-surface policy 没有先明确下来，后续工作就会反复重新做产品决策，而不是围绕既定契约推进实现

本 ADR 的目标不是列出所有 v3 breaking changes，而是明确**哪些事情已经决定**，让后续实现与 inventory 工作围绕同一套产品边界推进。
...
- 后续实现与文档不得把 “暂时继续保留 net8/net9，等以后再说” 当成默认策略
...
3. 后续 build 或 solution 调整不得把“由安装的 SDK 自动决定语言级别”当成可接受策略。
...
这里定义的是 **v3 coverage gate 的统计边界**，不是“哪些文件可以被测试”。
...
1. 仍然服务于 `docs/related/parity.md` 冻结的 capability scope。
...
允许的 public rename / removal rows 已经由 `docs/related/parity.md` 的“公开命名归一化映射”与相关 migration/release 文档共同承载。后续 parity、migration、release 文档必须按同一套发布规则写，不能再各自发明命名故事。

固定规则如下：

1. `docs/related/parity.md` 的 row key 继续锚定 upstream family identity，比如 `aiotieba.api.get_blacklist_old`、`aiotieba.api.get_replys`、`aiotieba.api.del_bawu`。
...
后续实现与文档工作一律使用以下 taxonomy，而不是再次争论边界。
...
| 后续工作单方面更改 migration guide 的章节骨架 | Forbidden | 迁移结构由本 ADR 定义。 |

## 7. 文档与 source-of-truth 层级

v3 文档层级定义为：

1. `docs/related/parity.md` = **parity truth**, including internal implementation mapping and auth notes
...
- `docs/related/parity.md` 保留 upstream family 可检索性；其 parity rows 使用规范化后的 public `C# Surface`，并在同一文档内承载 internal `Api/**` 映射与认证需求注记。
...
- 后续 docs / guide 工作必须围绕本 ADR 与 migration skeleton 对齐，而不是重新决定产品边界

## 8. 对 downstream work 的直接约束

| 工作范围 | 现在已经不需要重新决定的事情 | 约束结果 |
| --- | --- | --- |
...
本 ADR 之后，后续实现工作仍然很多，但它们不再拥有以下自由度：
...
这些事情已经在本 ADR 中定义；后续工作只负责实现、验证和记录具体 breaking inventory。

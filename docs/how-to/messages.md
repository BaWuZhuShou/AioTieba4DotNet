...
本页示例里的 `USER_ID_PLACEHOLDER`、`GROUP_ID_PLACEHOLDER` 等值统一遵循[示例占位符词汇表](/guide/getting-started#example-placeholder-glossary)。
...
```csharp
var groups = await client.Messages.GetGroupMessagesAsync(new long[] { long.Parse("GROUP_ID_PLACEHOLDER") }, getType: 1);
```
...
var id1 = await client.Messages.SendMessageAsync(long.Parse("USER_ID_PLACEHOLDER"), "MESSAGE_TEXT_PLACEHOLDER");
var id2 = await client.Messages.SendMessageAsync("USER_NAME_OR_PORTRAIT_PLACEHOLDER", "MESSAGE_TEXT_PLACEHOLDER");
...
await client.Messages.SendChatroomMessageAsync(
    chatroomId: long.Parse("CHATROOM_ID_PLACEHOLDER"),
    forumId: long.Parse("FORUM_ID_PLACEHOLDER"),
    text: "MESSAGE_TEXT_PLACEHOLDER",
    atUserIds: new long[] { long.Parse("USER_ID_PLACEHOLDER") });
```

...
public sealed class UserAndMessageSurfaceContractTests
{
    ...
    private static Assembly LoadLibraryAssembly()
    {
        ...
        {
            Path.Combine(
                RepositoryPaths.GetProjectDirectory(OnlineTestProjectTopology.Online),
                "bin",
                "Release",
                "net10.0",
                "AioTieba4DotNet.dll"),
        ...
        {
            Assert.Fail(
                $"Expected a built library assembly at one of: {string.Join(", ", candidatePaths)}. Build the Release online or library output before running this offline surface contract.");
        }

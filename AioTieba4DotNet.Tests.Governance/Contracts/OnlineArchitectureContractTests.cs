...
public sealed class OnlineArchitectureContractTests
{
    ...
    public void ProjectShellsContainMinimalSourceAndNewReferencesOnly()
    {
        ...
        CollectionAssert.AreEqual(
            ...
            ProjectFileSnapshot.Load(OnlineTestProjectTopology.Platform).ProjectReferences,
            "The platform support project should remain a plain class library with no project-to-project dependencies.");
        CollectionAssert.AreEqual(
            new[] { productReference, platformReference },
            ProjectFileSnapshot.Load(OnlineTestProjectTopology.Governance).ProjectReferences);

        foreach (var projectName in OnlineTestProjectTopology.ScenarioProjectNames)
        {
            CollectionAssert.AreEqual(
                new[] { productReference, platformReference },
                ProjectFileSnapshot.Load(projectName).ProjectReferences,
                $"Project '{projectName}' should reference only the product and platform owners.");
        }
    }
    ...
    public void FirstClassApiCategoriesResolveToDiscoverableRunnableScenarioTests()
    {
        CollectionAssert.AreEquivalent(
            ...
            "Matrix-backed first-class Api:* categories must map exactly to discoverable Online test usage with no missing or unexpected filter surface.");
        ...
        foreach (var category in PublicApiCoverageMatrixContract.AllowedFirstClassApiCategories)
        {
            ...
            Assert.IsTrue(
                tests.Length > 0,
                $"First-class Api:* category '{category}' must resolve to at least one discoverable runnable test in the Online scenario project.");
        }
    }

...
public sealed class MessagesModule : IMessagesModule
{
    ...
    public Task<bool> SendChatroomMessageAsync(long chatroomId, ulong forumId, string text,
        ...
        _protocol.SendChatroomMessageAsync(chatroomId, forumId, text, atUserIds, robotCode, cancellationToken);

    /// <inheritdoc />
    public Task<bool> SetMessageReadAsync(WsMessage message, CancellationToken cancellationToken = default) =>
        _protocol.SetMessageReadAsync(message, cancellationToken);

    /// <inheritdoc />
    public IReadOnlyList<WsNotify> ParsePushNotifications(byte[] payload) => _protocol.ParsePushNotifications(payload);
}

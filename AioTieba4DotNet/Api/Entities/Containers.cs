...
public class Containers<T> : IReadOnlyList<T>
{
    ...
    protected void Add(T item) => _objs.Add(item);

    // 支持 foreach 遍历（实现 IEnumerable<T>，也可继承 IReadOnlyList<T>）
    public IEnumerator<T> GetEnumerator() => _objs.GetEnumerator();
    IEnumerator IEnumerable.GetEnumerator() => _objs.GetEnumerator();

    // 支持索引访问
    public T this[int index] => _objs[index];

    // 不允许 set、del，显式抛出异常
    public void SetItem(int index, T value) => throw new NotImplementedException();
